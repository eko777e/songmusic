...
async def download_cb(client, query):
    ...
    m = await app.send_message(chat_id=query.message.chat.id, text="`🚀Musiqi endirilir...`")
    ...
    await m.edit("`⚡Musiqi göndərilir...`")
    ...
    await app.send_audio(chat_id=query.message.chat.id, audio=audio_file, performer="@UzeyirMusic_Bot", thumb=thumb_name, duration=duration_seconds, caption=f"📎Adı: [{title}]({link})\n\n🎵Yüklədi: @UzeyirMusic_Bot", reply_markup=channel_markup)
    await m.delete()
    ...
    if share:
        await app.send_audio(chat_id=PLAYLIST_ID, audio=audio_file, performer="@UzeyirMusic_Bot", thumb=thumb_name, duration=duration_seconds, caption=f"🎤Adı: [{title}]({link})\n\n🎵Yüklədi: @UzeyirMusic_Bot")
